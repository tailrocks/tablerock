...
| Column controls | Parity | Show/hide, reorder, width, fit, format, one-action reset, and stable per-table preferences | Toggle/reset + persist (225); ColL/ColR reorder + Col± width on layout-aware VirtualGrid (evidence 347); fit/format polish open |
...
| Filtering | Core | Filter bar with typed per-column conditions plus a raw-WHERE mode; clearly labeled resident-page quick filter; saved presets later | Phase 5 parameterized filters + page-local quick filter (evidence 223–225); visual chip bar polish still open; saved presets Phase 9 |
...
| Row/value inspector | Core | Full typed values, raw bytes/hex, JSON text/tree, metadata, and stale state | Phase 4 inspector panel text/hex/metadata/stale (evidence 213); structured JSON tree + depth collapse (evidence 339); interactive expand still optional |
| Type-specific editors | Parity | Bool, number, temporal, enum, JSON, bytes, array/map/tuple, and explicit unknown fallback | Partial: paste buffer + parse heuristics (bool/int/float/text/null); ValueKind-specific widgets still open (evidence 230) |
