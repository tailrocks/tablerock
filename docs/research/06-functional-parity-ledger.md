...
| Query/command tabs | Core | Independent text, cursor, context, results, errors, history, and running operation | Phase 2 now proves typed command scopes, finite budgets, scoped operation identity, legal lifecycle/cancellation-outcome edges, stale/duplicate rejection, gap resync, and one core-authoritative application-service harness over real bounded PostgreSQL, ClickHouse, and Redis tasks; execution text, multi-tab behavior, overlapping-operation budgets, and server cancellation races remain required |
