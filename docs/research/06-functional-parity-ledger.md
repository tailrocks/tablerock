...
| Profile organization | Parity | Groups, tags, favorites, ordering, filters, and environment/safety markers without copied reference presentation | Phase 2 proves versioned/redacted sources, immutable connection snapshots, saved/temporary disposition, bounded groups/tags/favorite/order/preferences, and non-consuming revision CAS validation; search/filter UI, CRUD actor, transactional enforcement, migration, resolution, and persistence remain required |
