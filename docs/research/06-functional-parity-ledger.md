...
| Query/command tabs | Core | Independent text, cursor, context, results, errors, history, and running operation | Multi-tab cancellation and stale-event tests |
...
| Paging | Core | Bounded server pages for objects and bounded streams for arbitrary queries; totals may be estimated/unknown | Page race, eviction, and count-budget tests |
