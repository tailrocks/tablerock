...
| Typed display | Core | Distinguish NULL, empty, whitespace, zero, false, bytes, unknown, invalid, and truncated values | The Phase 2 owned-value unit contract covers null/empty/whitespace/zero/false/bytes/unknown/invalid/truncated distinctions; real ClickHouse 25.8/26.3 LTS fixtures now prove booleans, all scalar integer widths through 256 bits, Decimal256, floats, temporal types, UUID/IP, enums, low-cardinality text, nullable text, binary, and bounded unknown fallback; PostgreSQL/Redis breadth and ClickHouse containers remain required |
...
| Paging | Core | Bounded server pages for objects and bounded streams for arbitrary queries; totals may be estimated/unknown | The Phase 2 immutable-page unit contract covers pre-allocation dimensions, ranges, offsets, null/truncation metadata, unknown totals, hostile buffers, and checked row-major adapter assembly; PostgreSQL 17/18, ClickHouse 25.8/26.3, and Redis 7.4/8.8 Testcontainers now prove bounded driver pages/streams, while page races, eviction, count budgets, bridge encoding, and the complete operation matrix remain required |
