...
| Profile organization | Parity | Groups, tags, favorites, ordering, filters, and environment/safety markers without copied reference presentation | Phase 2 proves versioned/redacted source shapes plus bounded duplicate-free property bindings that forbid ordinary literal password/private-key material; profile CRUD, complete snapshot/readiness, migration, ordering, stale revision, resolution, and persistence remain required |
