...
| Query/command tabs | Core | Independent text, cursor, context, results, errors, history, and running operation | Phase 2 now proves typed command scopes, finite budgets, scoped operation identity, legal lifecycle/cancellation-outcome edges, stale/duplicate rejection, gap resync, one core-authoritative application-service harness with simultaneous bounded PostgreSQL, ClickHouse, and Redis tasks, PostgreSQL SQLSTATE cancellation truth, and ClickHouse synchronous query-ID cancellation truth; execution text, multi-tab behavior, published performance budgets, Redis cancellation, and remaining server races are required |
