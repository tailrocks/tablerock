...
| Query/command tabs | Core | Independent text, cursor, context, results, errors, history, and running operation | Phase 2 now proves typed command scopes, finite budgets, scoped operation identity, legal lifecycle/cancellation-outcome edges, stale/duplicate rejection, gap resync, and a core-authoritative application-service bridge over bounded engine tasks with real PostgreSQL execution; execution text, multi-tab engine behavior, cross-engine service evidence, and server cancellation races remain required |
