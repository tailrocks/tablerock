...
| Profile organization | Parity | Groups, tags, favorites, ordering, filters, and environment/safety markers without copied reference presentation | Phase 2 proves versioned/redacted sources, literal-secret prohibition, immutable connection sub-snapshots, all-engine host/port readiness, closed TLS state, two-mode selection, and finite limits; complete aggregate/preferences, CRUD, organization, enforcement, migration, stale revision, resolution, and persistence remain required |
