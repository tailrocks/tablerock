...
| Phase 2 — PostgreSQL cancel transport loss | [`156-phase-2-postgresql-cancel-transport-loss.md`](156-phase-2-postgresql-cancel-transport-loss.md) | Reviewed 2026-07-17. PostgreSQL 17.10/18.4 force-stop fixtures prove server loss before the separate cancel socket opens yields redacted cancellation-transport failure and terminal session connection loss, never false server-confirmed cancellation. Post-delivery outcome loss, page-delivery races, reconnect, ambiguous writes, notices, parameters, multiple statements, COPY, typed breadth, and presentation remain open. |
