...
| Paging | Core | Bounded server pages for objects and bounded streams for arbitrary queries; totals may be estimated/unknown | The Phase 2 immutable-page unit contract covers pre-allocation dimensions, ranges, offsets, null/truncation metadata, unknown totals, hostile buffers, and checked row-major adapter assembly; a PostgreSQL 18.4 text-stream tracer proves 2+1 page delivery with scalar-safe truncation, but page races, eviction, count budgets, typed driver values, bridge encoding, and the complete real-server matrix remain required |
