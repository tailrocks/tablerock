...
The live loop accepts the root receiver rather than constructing unreachable
placeholder plumbing. OS shutdown signals are selected first and cannot be
starved by queue or terminal traffic. Typed engine mapping, progress coalescing,
and explicit resync on overflow remain required before that Phase 1 gate is
complete.
...
- A process contract test fills the injected post-mapping root port to exactly
  256 messages and proves the next non-blocking send is rejected.
