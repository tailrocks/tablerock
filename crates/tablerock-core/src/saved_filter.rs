//! Named filter presets shared by every presentation adapter.
//!
//! Persistence stores JSON only; engine re-types values when re-browsing.

/// One persisted typed-filter intent. Display text is re-typed by the engine.
...
pub struct SavedFilterPreset {
    ...
    pub schema: String,
    pub table: String,
    pub filters: Vec<SavedFilterCondition>,
        ...
        while let Some(idx) = rest.find(r#""name""#) {
            ...
            if let Some(fstart) = rest.find(r#""filters""#) {
                ...
                while let Some(cidx) = frest.find(r#""column""#) {
                    ...
                    filters.push(SavedFilterCondition {
                        column,
                        operator,
                        value,
                    });
...
mod tests {
    ...
    fn round_trip_preset_json() {
        ...
        lib.upsert(SavedFilterPreset {
            ...
            schema: "public".into(),
            table: "users".into(),
            filters: vec![SavedFilterCondition {
                column: "status".into(),
                operator: "eq".into(),
