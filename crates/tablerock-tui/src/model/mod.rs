...
pub enum ActionId {
    ...
    PrevDirtyTab,
    /// Select first dirty workbench tab (fail closed if none dirty).
    FirstDirtyTab,
    /// Select last dirty workbench tab (fail closed if none dirty).
    LastDirtyTab,
    CloseTab,
    ...
    CloseTabsToLeft,
    /// Close every workbench tab (fails closed if any dirty).
    CloseAllTabs,
    /// Close every preview tab (previews are never dirty).
    CloseAllPreviewTabs,
    ...
    CopyTabCounts,
    /// Copy preview tab titles (one per line); fail closed if none.
    CopyPreviewTabTitles,
    PinTab,
    NewSql,
    RunSql,
    ...
    CopyQuickFilter,
    /// Copy visible/hidden/total column counts summary.
    CopyColumnCounts,
    ...
    CopySchema,
    /// Copy base schema as a SQL-quoted identifier.
    CopySchemaIdent,
    ...
    ReverseFilters,
    /// Promote newest filter chip to front and re-browse.
    PromoteLastFilter,
    /// Demote oldest filter chip to end and re-browse.
    DemoteFirstFilter,
    ...
    FilterILike,
    /// Add NOT LIKE filter (cell text with % wildcards).
    FilterNotLike,
    /// Add NOT ILIKE filter (case-insensitive).
    FilterINotLike,
    /// LIKE prefix filter: cell text + '%' (starts with).
    FilterStartsWith,
    /// LIKE suffix filter: '%' + cell text (ends with).
    FilterEndsWith,
    /// ILIKE prefix filter (case-insensitive starts with).
    FilterIStartsWith,
    ...
    FilterNotEndsWith,
    /// NOT ILIKE prefix filter (case-insensitive does not start with).
    FilterINotStartsWith,
    /// NOT ILIKE suffix filter (case-insensitive does not end with).
    FilterINotEndsWith,
    ...
    /// Copy staged status suffix (`staged N (i↑ c· d↓)`); fail closed if empty.
    CopyStagedStatus,
    ...
    /// Step temporal date by -1 year while editing.
    DecYear,
    /// Step temporal time by +1 hour while editing.
    IncHour,
    /// Step temporal time by -1 hour while editing.
    DecHour,
    /// Step temporal time by +1 minute while editing.
    IncMinute,
    /// Step temporal time by -1 minute while editing.
    DecMinute,
