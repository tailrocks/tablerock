...
fn activate_selected_action(model: &mut Model) -> Update {
    match model.selected_action() {
        ...
        ActionId::FirstDirtyTab if model.screen() == Screen::Workbench => {
            if model.workbench_mut().select_first_dirty_tab() {
                Update::render()
            } else {
                Update::unchanged()
            }
        }
        ActionId::LastDirtyTab if model.screen() == Screen::Workbench => {
            if model.workbench_mut().select_last_dirty_tab() {
                Update::render()
            } else {
                Update::unchanged()
            }
        ...
        ActionId::CopySchemaIdent if model.screen() == Screen::Workbench => {
            use crate::model::structure_ddl::quote_ident_sql;
            let Some(grid) = model.workbench().active_grid() else {
                return Update::unchanged();
            };
            ...
            let token = model.mint_request_token();
            if let Some(g) = model.workbench_mut().active_grid_mut() {
                g.error_label = Some(format!("copied schema ident {text}"));
            }
            Update {
                render: true,
                effect: Some(Effect::CopyToClipboard {
        ...
        ActionId::InvertPrimarySort if model.screen() == Screen::Workbench => {
            let flipped = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.invert_primary_sort());
            if !flipped {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        ...
        ActionId::RotateSortRight if model.screen() == Screen::Workbench => {
            let rotated = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.rotate_sort_keys_right());
            if !rotated {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::KeepPrimarySort if model.screen() == Screen::Workbench => {
            let kept = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.keep_primary_sort());
            if !kept {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::PromoteSort if model.screen() == Screen::Workbench => {
            let col = model
                .workbench()
                .active_grid()
                .and_then(|g| g.columns.get(g.cursor_col).cloned());
            ...
            let promoted = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.promote_sort_column(&col));
            if !promoted {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::InvertAllSort if model.screen() == Screen::Workbench => {
            let flipped = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.invert_all_sorts());
            if !flipped {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::SwapSortKeys if model.screen() == Screen::Workbench => {
            let swapped = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.swap_primary_secondary_sort());
            if !swapped {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::ReverseSortKeys if model.screen() == Screen::Workbench => {
            let reversed = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.reverse_sort_keys());
            if !reversed {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        ...
        ActionId::RemoveFirstFilter if model.screen() == Screen::Workbench => {
            let removed = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.remove_first_filter());
            if !removed {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        }
        ActionId::ReverseFilters if model.screen() == Screen::Workbench => {
            let reversed = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.reverse_filters());
            if !reversed {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        ...
        ActionId::DemoteFirstFilter if model.screen() == Screen::Workbench => {
            let demoted = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.demote_first_filter());
            if !demoted {
                return Update::unchanged();
            }
            rebrowse_active_table(model)
        ...
        ActionId::GoToFirstStaged if model.screen() == Screen::Workbench => {
            let moved = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.go_to_first_staged());
            if moved {
                Update::render()
            } else {
                Update::unchanged()
        ...
        ActionId::GoToLastStaged if model.screen() == Screen::Workbench => {
            let moved = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.go_to_last_staged());
            if moved {
                Update::render()
            } else {
                Update::unchanged()
        ...
        ActionId::EqualizeColumnWidths if model.screen() == Screen::Workbench => {
            let changed = model
                .workbench_mut()
                .active_grid_mut()
                .is_some_and(|g| g.equalize_visible_column_widths());
            if changed {
                Update::render()
            } else {
                Update::unchanged()
        ...
        | ActionId::FirstDirtyTab
        | ActionId::LastDirtyTab
        | ActionId::CloseTab
        ...
        | ActionId::RotateSort
        | ActionId::RotateSortRight
        | ActionId::KeepPrimarySort
        | ActionId::PromoteSort
        | ActionId::InvertAllSort
        | ActionId::SwapSortKeys
        | ActionId::ReverseSortKeys
        | ActionId::ClearSort
        | ActionId::AddFilter
        ...
        | ActionId::GoToFirstStaged
        | ActionId::GoToLastStaged
        | ActionId::PageUp
        | ActionId::PageDown
        | ActionId::HalfPageUp
...
mod tests {
    ...
    fn copy_schema_and_bare_table_emit_parts() {
        ...
        match update(&mut model, Message::Activate).effects().next() {
            Some(Effect::CopyToClipboard { text, .. }) => assert_eq!(text, "\"analytics\""),
            other => panic!("expected schema ident, got {other:?}"),
        }
