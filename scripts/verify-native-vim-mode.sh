...
for pattern in \
  ...
  'guard event.keyCode == 53, !hasMarkedText\(\)' \
  'case "h": moveLeft\(nil\)' \
  'case "j": moveDown\(nil\)' \
...
do
  rg -q "$pattern" "$SOURCE" || {
    echo "error: missing native Vim contract: $pattern" >&2
    exit 1
  }
done
...
audit_log="$(mktemp "$REPO_ROOT/target/native-vim-mode.XXXXXX")"
open -n -F --env TABLEROCK_FIXTURE_VIM_MODE_AUDIT=1 \
...
for _ in $(seq 1 50); do
  rg -q '^VIM_MODE_PROOF_' "$audit_log" && break
  sleep 0.1
done
if ! rg -q '^VIM_MODE_PROOF_PASSED ' "$audit_log"; then
  cat "$audit_log" >&2
  echo "error: native Vim runtime proof failed" >&2
  exit 1
fi

echo "native Vim structural and runtime gate passed"
