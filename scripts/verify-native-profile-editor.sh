...
audit_log="$(mktemp "$REPO_ROOT/target/native-profile-editor.XXXXXX")"
open -n -F --env TABLEROCK_FIXTURE_PROFILE_EDITOR=1 \
...
for _ in $(seq 1 50); do
  rg -q '^PROFILE_EDITOR_PROOF_' "$audit_log" && break
  sleep 0.1
done
if ! rg -q '^PROFILE_EDITOR_PROOF_PASSED ' "$audit_log"; then
  cat "$audit_log" >&2
  echo "error: native profile editor runtime proof failed" >&2
  exit 1
fi

echo "native profile editor structural and runtime gate passed"
