...
Column reorder/resize (347) + fit-to-content (352) on VirtualGrid.
Redis command editor mirrored via plan 015.

## Phase 6 — PostgreSQL write/admin slice

**Mostly complete (evidence 227–234, 323–327, 339–340):** write path,
editability, staged drafts, typed review, RETURNING, consume-once registry
+ re-review on expiry, FK/structure with index + constraint defs,
...
**Software and XCFramework gates complete (plan 019):** page v1 codec, `tablerock-ffi` facade,
UniFFI Swift bindings, conformance (stubs + Docker PG/CH/Redis), review/apply
handles, profile open, universal XCFramework, Swift `PageV1` decode, and proof
harness are on `main`. **Distribution gate operator-blocked:** Developer ID
signing + notarize/staple/clean-machine (see evidence 251 and 406). Phase 13's
locally workable application exists; release proof still waits on the
distribution gate.
...
**Complete (evidence 407–414, 505–511, 629):** the locally runnable strict Swift 6
...
appearance, responsive inspector/control reachability, 10k-grid, page-decode,
RSS, and leak gates. Developer ID release
