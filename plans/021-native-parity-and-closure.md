...
- **IN PROGRESS (2026-07-22): native connection workflows (evidence 512–518), history/retention (519–520), saved queries (521), SQL files (522), typed intent bridge (523), query tabs (524), read-only preview/pinned object tabs (525), multi-window restoration (526), environment/safety projection (527), typed value inspection (528), shared Rust/native multi-format copy (529), native atomic loaded-result export (530), shared bounded CSV import foundation (531), native reviewed CSV import (532), PostgreSQL/ClickHouse structure (533–534), bounded Redis key catalog projection (535), native Redis key object views (536), native Redis sampled overview (537), SwiftPM bridge regression foundation/boundary expansion (538–539), deterministic cross-engine PageV1 fixtures (540), typed native test-root isolation (541), stable automation identifiers (543), native clock/identity ports (544), isolated file-panel/pasteboard ports (546), native Keychain resolution (549), live backend protocol isolation (552), scripted backend failure seams (553), hosted XCTest/XCFramework/app checkpoint gate (562), application-owned immutable backend DTOs (563), canonical hosted Xcode checkpoint/archive proof (564), scheduled Nightly/performance/artifact metadata proof (565), the Phase 15 safe support-bundle contract/CLI foundation (566), native per-object ordered multi-sort, typed server filters, raw-WHERE controls, saved presets, resident-page quick filtering, sort-header projection, direct control activation, reviewed connection URL import, and external URL authority (611–615, 617–618, 626, 629–631), plus canonical screen-manifest structural validation, two explicit missing-surface expansions, and per-client status validation (624–625, 627–628) landed; testing-system checkpoints 12–13 are complete, checkpoint 14's deterministic checkpoint and unsigned scheduled performance foundation are complete while its live/IME/full-accessibility matrix remains open, checkpoint 15 has an exact unsigned shipping shape but signed clean-machine proof remains externally blocked, checkpoint 16 has checkpoint and Nightly artifacts, and Phase 15 diagnostics and manifest replay remain partial; broader import types/engines, full streaming export, advanced object state, and remaining screens continue**
- **Recent checkpoints**: external URL authority, stable-ID quick switching,
  and shared Rust-owned PostgreSQL activity/signal contracts landed (evidence
  631–632, 635); their hosted edge-state replay remains.
- **Current checkpoint**: native PostgreSQL backup/restore uses shared
...
8. **Support matrix + diagnostics — safe bundle foundation complete (evidence 566)**: tested server/terminal/macOS/
...
16. **CI and evidence artifacts — checkpoint/Nightly foundation complete (evidence 564–565)**
